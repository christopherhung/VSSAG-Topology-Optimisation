"""
adjoint.py
Executes the exact discrete adjoint bypass to cap GPU physics footprint at 0.0 MB.
"""
import numpy as np
...
def solve_kinematics_cpu(K_sparse: sps.csc_matrix, F_vector: np.ndarray) -> np.ndarray:
    """
    Executes the direct LU factorization.
    Iterative solvers are bypassed to avoid instability from void regions (E_min).
    Matrix factorization occurs entirely in System RAM.
    """
    return spsolve(K_sparse, F_vector)

class AdjointCompliance(torch.autograd.Function):
    """
    Analytically injects exact discrete structural sensitivity back into PyTorch.
    """
    @staticmethod
    def forward(
        ctx: Any, 
        rho_phys: torch.Tensor, 
        freedofs: np.ndarray, 
        fixeddofs: np.ndarray, 
        F_csc: sps.csc_matrix
    ) -> torch.Tensor:
        ...
        # Base Element Stiffness Matrix (KE) Assembly
        ...
        edofMat = np.tile(edofVec, (1, 8)) + np.tile(
            np.array([0, 1, 2*PHYSICS.nely+2, 2*PHYSICS.nely+3, 2*PHYSICS.nely, 2*PHYSICS.nely+1, -2, -1]), 
            (PHYSICS.nelx*PHYSICS.nely, 1)
        )
        iK = np.kron(edofMat, np.ones((8, 1))).flatten()
        jK = np.kron(edofMat, np.ones((1, 8))).flatten()
        
        # Direct Sparse Assembly & Kinematic Solve
        ...
        U = np.zeros(ndof)
        U[freedofs] = solve_kinematics_cpu(K[freedofs, :][:, freedofs], F_csc[freedofs, :])
        
        # Local Element Strain Energy (ce) & Global Compliance
        U_e = U[edofMat]
        ce = np.sum(np.dot(U_e, KE) * U_e, axis=1)
